...
if __name__ == "__main__":
    ...
    # overhang = 2.5*ft
    for sect in hayward8, hayward6, hayward_single, painter, meloland, crowley:
        mesh = sect2gmsh(sect, [inch]*2)
        solution = torsion.solve_torsion(sect, mesh)
        torsion.plot(mesh, solution, scale=1.0, show_edges=True)
        render.section(sect)#, set_limits=True)
        render.show()
