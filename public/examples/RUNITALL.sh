...
export OPENSEESRT_LIB=/home/claudio/packages/opensees-pypi/build/temp.linux-x86_64-cpython-311_local/src/libg3/SRC/runtime/libOpenSeesRT.so
...
#export OPENSEES=/home/claudio/packages/opensees-pypi/build/temp.linux-x86_64-cpython-39_local/src/libg3/SRC/executable/OpenSees
...
for i in */test.tcl; do
  ...
  cd $dir
  echo "FILE $i"; 
  msg="$($OPENSEES test.tcl 2>&1 >/dev/null)";
  code="$?"
  echo "$msg"
  msg=$(tail -1 <<< "$msg")
