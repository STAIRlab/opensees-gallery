...
######################################################################
#  Environment: Successfully excucted in python 3.8
...
class SqliteDB(object):
    ...
    def saveNodes(self, nodesSaveName, nodeList):
        ...
        nodesDict = [
            {"tags": int(each[0]), "contents": str(each[1:])} for each in nodeList
        ]
        nodesTable = f"""
    ...
    def saveEles(self, elesSaveName, elesList):
        ...
        db = records.Database("sqlite:///" + self._dbPath)
        elesDict = [
            {"tags": int(each[0]), "contents": str(each[1:])} for each in elesList
        ]
        nodesTable = f"""
                        CREATE TABLE IF NOT EXISTS
                        {elesSaveName}(
    ...
    def saveModes(self, modesName, modesList):
        ...
        db = records.Database("sqlite:///" + self._dbPath)
        nodesDict = [
            {"tags": int(each[0]), "contents": str(each[1:])} for each in modesList
        ]
        nodesTable = f"""
                                CREATE TABLE IF NOT EXISTS
                                {modesName}(
    ...
    def savePeriod(self, periodList):
        ...
        db = records.Database("sqlite:///" + self._dbPath)
        periodDict = [
            {"tags": int(each[0]), "contents": str(each[1:])} for each in periodList
        ]
        periodTable = f"""
                                        CREATE TABLE IF NOT EXISTS
                                        periods(
    ...
    def saveGeomTransf(self, geomTransfSaveName, geomfList):
        ...
        db = records.Database("sqlite:///" + self._dbPath)
        geomfDict = [
            {"tags": int(each[0]), "contents": str(each[1:])} for each in geomfList
        ]
        geomfTable = f"""
                                CREATE TABLE IF NOT EXISTS
                                {geomTransfSaveName}(
    ...
    def saveEleLocalCoordSys(self, SaveName, EleLocalCoordSys):
        ...
        db = records.Database("sqlite:///" + self._dbPath)
        geomfDict = [
            {"tags": str(each[0]), "contents": str(each[1:])}
            for each in EleLocalCoordSys
        ]
        geomfTable = f"""
                                        CREATE TABLE IF NOT EXISTS
                                        {SaveName}(
    ...
    def getNodeTimeHistory(self, nodeTag, resType, dof):
        ...
        responseList = []
        [
            [timesList.append(each["times"]), responseList.append(each[dofDict[dof]])]
            for each in returnValue
        ]
    ...
    def saveTrussEleResponseTimeHistory(self, eleSaveName, eleHistoryList):
        ...
        """
        eleResponseDict = [
            {"times": float(each[0]), "resValue": float(each[1])}
            for each in eleHistoryList
        ]
        nodesTable = f"""
                                CREATE TABLE IF NOT EXISTS
                                {eleSaveName}(
    ...
    def getTrussEleResponseTimeHistory(self, eleTag, resType):
        ...
        responseList = []
        [
            [timesList.append(each["times"]), responseList.append(each["resValue"])]
            for each in returnValue
        ]
        return timesList, responseList
    ...
    def saveZeroEleResponseTimeHistory(self, eleSaveName, eleHistoryList):
        ...
        linkstr += (
            f"'dof_{directions[-1]}':float(each[1][-1])}} for each in {eleHistoryList}]"
        )
    ...
    def getZeroEleResponseTimeHistory(self, eleTag, resType, dof):
        ...
        try:
            queryValue = conn.query(
                f"""select times,{dofDict[dof]} from {tableName};"""
            )
            returnValue = queryValue.all(as_dict=True)
        except:
            print(f"""Something is wrong. Please check the parameters!""")
        ...
        eleResponseDict = [
            {
                ...
                "dofJ_2": float(each[8]),
                "dofJ_3": float(each[9]),
                "dofJ_4": float(each[10]),
                "dofJ_5": float(each[11]),
                "dofJ_6": float(each[12]),
            }
            for each in eleHistoryList
        ]
        nodesTable = f"""
    ...
    def getNonZeroEleResponseTimeHistory(self, eleTag, resType, eleEnd, dof):
        ...
        try:
            queryValue = conn.query(
                f"""select times,{dofDict[inqTag]} from {tableName};"""
            )
            returnValue = queryValue.all(as_dict=True)
        except:
            print(f"""Something is wrong. Please check the parameters!""")
        ...
        [
            [
                timesList.append(each["times"]),
                responseList.append(each[dofDict[inqTag]]),
            ]
            for each in returnValue
        ]
        return timesList, responseList
...
if __name__ == "__main__":
    dbPath = "output/resultsDB_1_114.db"
