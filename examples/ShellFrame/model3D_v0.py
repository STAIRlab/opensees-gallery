...
def get_last_element_tag():
    ...
    if all_ele_tags:  # Check if the list is not empty
        ...
        return 0  # Return None if no nodes have been defined
def find_closest_node(midpoint, node_tags):

    # Loop through all node tags to find the closest node
    for node_id in node_tags:
        coord = np.array(ops.nodeCoord(node_id))
...
def create_slabs(corner_nodes, current_ele_id, slab_id, slab):
    """
    ...
    for i in range(0, len(corner_nodes), 4):
        if i + 3 < len(corner_nodes): 
            ...
            for j in range(4):
                ...
                ops.mesh('line', line_tag, 2, *[start_node, end_node], 0, 6, slab.mesh_size)
                line_tags.append(line_tag)
                current_ele_id += 1
                
              
                node_coords.append(np.array(ops.nodeCoord(start_node)))

            
            ops.section('ElasticMembranePlateSection', slab_id, slab.E, slab.nu, slab.h, slab.rho)

            # Create the shell element using the line meshes
            ops.mesh('quad', current_ele_id, 4, *line_tags, 0, 6, slab.mesh_size, 'ShellMITC4', 1)
            current_ele_id += 1
            slab_id += 1
            ...
            all_node_tags = ops.getNodeTags()
            closest_node = find_closest_node(midpoint, all_node_tags)
...
def eigenvalue_analysis(n_modes, plot_mode_shapes):
    ...
    """
    lambdaN = ops.eigen(n_modes)  # eigenvalue analysis for nEigenJ modes
...
def model3D(modify, node_id, length_x, length_y, length_z, base_z, spacing, num_bay_x, num_bay_y, num_story, eleTag,
                ...
                if level < 1:  # Adjust if more levels are needed
                    for corner in [(ix * length_x, iy * length_y, z),
                        ...
                        if upper_corner not in node_map:
                            ops.node(current_node_id, *upper_corner)
                        ...
                        if column_key not in column_set:
                            ...
                            XYZI = ops.nodeCoord(node_map[corner])
                            XYZJ = ops.nodeCoord(node_map[upper_corner])
                            ...
                            # Define the geometric transformation with the appropriate geoTag
                                ops.geomTransf('Linear', geoTag, *vecxz)

                            
                            ops.element('elasticBeamColumn', current_ele_id, node_map[corner], node_map[upper_corner],
                                    column.A, column.E, column.G, column.J, column.Iweak, column.Istrong, geoTag, '-mass', 463)
                            current_ele_id += 1
            ...
            for side in range(4):
                ...
                for i in range(num_steps):
                    ...
                    if prev_node is not None and tuple(sorted([prev_node, new_node_id])) not in beam_set:
                        # Local x-axis from nodal coordinates
                        XYZI = ops.nodeCoord(prev_node)
                        XYZJ = ops.nodeCoord(new_node_id)
                        xaxis = np.subtract(XYZJ, XYZI)
                        ...
                        ops.element('elasticBeamColumn', current_ele_id, prev_node, new_node_id, beam.A, beam.E, beam.G, beam.J, beam.Iweak, beam.Istrong, geoTag, '-mass', 90)
...
def dynamic_analysis(spacing, num_bay_x, num_bay_y, num_story, target_coord,
    ...
    for i in range(0, num_story*length_z, length_z):
        current_node_id, current_ele_id, corner_nodes, geoTag_counter = model3D(modify, node_id, length_x, length_y, length_z, i, spacing, num_bay_x, num_bay_y, num_story, eleTag, 
                                              geoTag_counter, vecxz_to_geoTag,
                                              W14X311, W10X60,
                                              node_map, column_set, beam_set,corners,corner_nodes)
    ...
    for node_id in unique_nodes:
        ops.mass(node_id, m1, m1, m1, 1.0e-10, 1.0e-10, 1.0e-10)
    ...
    current_ele_id, slab_id, closest_nodes = create_slabs(corner_nodes, eleTag, slab_id, slabs)
    
    
    target_node_id = target_node(target_coord)
    
    for ndI in ops.getNodeTags():
        XYZI = np.array(ops.nodeCoord(ndI))
        for ndJ in ops.getNodeTags():
            if ndI >= ndJ:
                continue
            XYZJ = np.array(ops.nodeCoord(ndJ))
            if np.linalg.norm(XYZJ-XYZI) < 1e-8:
                ops.equalDOF(ndI,ndJ,1,2,3,4,5,6)
    
    slab_loc = ops.nodeCoord(target_node_id)
    
    slab_nodetag = target_node_id
    allTags = ops.getNodeTags()
    nsc_nodetag = allTags[-1] + 1
    
    L_nsc = 1.0 # m
    
    ops.node(nsc_nodetag, slab_loc[0], slab_loc[1], slab_loc[2]+L_nsc)
    ...
    all_eletag =ops.getEleTags()
    nsc_eletag = all_eletag[-1] + 1
    
    # Local x-axis from nodal coordinates
    XYZI = ops.nodeCoord(slab_nodetag)
    XYZJ = ops.nodeCoord(nsc_nodetag)
    xaxis = np.subtract(XYZJ, XYZI)
    ...
    # Define the geometric transformation with the appropriate geoTag
        ops.geomTransf('Linear', geoTag, *vecxz)
    ops.element('elasticBeamColumn', nsc_eletag, slab_nodetag, nsc_nodetag,axial_area_nsc, E_nsc, G_nsc, Jxx_nsc, Iy_nsc, Iz_nsc, geoTag)
    
    ops.mass(nsc_nodetag, 1.0e-10,1.0e-10,m_nsc , 1.0e-10, 1.0e-10, 1.0e-10)
    ...
    G = gm_sf * 9.81
    ops.timeSeries('Path', timeSeries_tag, '-dt', dt, '-values', *eq_data, '-factor', G)
    ops.pattern('UniformExcitation', pattern_tag, GM_direction, '-accel', timeSeries_tag)
    ...
    # NSC
    ops.recorder('Node','-file', NSCacc_path ,'-timeSeries',timeSeries_tag,'-time', '-node', nsc_nodetag, '-dof',3, 'accel' )
    
    # Slab
    ops.recorder('Node','-file',slabacc_path,'-timeSeries', timeSeries_tag,'-time', '-node', slab_nodetag, '-dof',3, 'accel' )
