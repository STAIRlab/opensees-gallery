...
def build_RC_rect_section(model, sec_tag, H, B, cover_H, cover_B, core_tag,
    ...
    model.layer('straight', steel_tag, n_bars_int, bar_area_int,
          *[-core_y, core_z], *[core_y,core_z])
    ## intermediate skin reinf. -z
    model.layer('straight', steel_tag, n_bars_int, bar_area_int,
          *[-core_y, -core_z], *[core_y, -core_z])
    ## top layer reinfocement
    model.layer('straight', steel_tag, n_bars_top, bar_area_top,
            *[core_y, core_z], *[core_y, -core_z])
    ## bottom layer reinforcement
    model.layer('straight', steel_tag, n_bars_bot, bar_area_bot,
      *[-core_y, core_z], *[-core_y, -core_z])
...
def build_model(L_X, L_Y, L_Z,
    ...
    model.uniaxialMaterial(*mat_KSP_unconf.values())
    model.uniaxialMaterial(*mat_KSP_conf.values())
    ## For reinforcing steel
    model.uniaxialMaterial(*mat_GMP.values())



    # Adding Sections
    ## For Columns
    build_RC_rect_section(model, 1,
                             sections['Column']['H'],
                             sections['Column']['B'],
    ...
    model.geomTransf('PDelta', trans_tags['P_delta'], *[-1.0, 0.0, 0.0])
    model.geomTransf('Linear', trans_tags['linear_beam_X'], *[0.0, 1.0, 0.0])
    model.geomTransf('Linear', trans_tags['linear_beam_Y'], *[1.0, 0.0, 0.0])
    ...
    for e in all_the_beams_X:
        model.element('nonlinearBeamColumn', e, *connectivity[e-1],
    ...
    for n in loaded_nodes:
        model.load(n, *[0,0,-C_L,0,0,0])
        model.mass(n, *[m_1*0.25, m_1*0.25,0, 0,0,0])
...
def run_gravity(model, steps = 10):
    ...
    model.test('NormDispIncr', 1.0e-6, 100, 0, 2)
...
def run_modal(model, n_evs = 3):
    ...
    model.recorder('Node', '-file',
            os.path.join('assets', 'ModalAnalysis_Node_EigenVectors_EigenVec1.out'),
                 '-node', *list(range(5,9)), '-dof', *[1,2], 'eigen 1')
    model.recorder('Node', '-file',
            os.path.join('assets', 'ModalAnalysis_Node_EigenVectors_EigenVec2.out'),
                 '-node', *list(range(5,9)), '-dof', *[1,2], 'eigen 2')

    model.recorder('Node', '-file',
            os.path.join('assets', 'ModalAnalysis_Node_EigenVectors_EigenVec3.out'),
    ...
    # Uses the norm of the left hand side solution vector of the matrix equation to
    # determine if convergence has been reached
    model.test('NormDispIncr', 1.0e-12, 25, 0, 2)
    ...
    # Eigenvalue analysis
    lamda = np.array(model.eigen(n_evs))

    # Writing Eigenvalue information to file
    with open(os.path.join('assets', 'ModalAnalysis_Node_EigenVectors_EigenVal.out'), "w") as eig_file:
...
def run_pushover(model, m_1, steps = 10000, direction = 'X'):
    """
    ...
    model.integrator('DisplacementControl',5, d_o_f, step)
...
def run_time_history(model, direction = 'X', g_motion_id = 1, scaling_id = 1,
                     lamda = 1, acc_dir = 'assets/acc_1.txt',alpha = 0.29):
    """
    ...
    if direction == 'X':
        ...
        omega = np.loadtxt(os.path.join('assets', 'ModalAnalysis_Node_EigenVectors_EigenVal.out'),skiprows=1)[0,1]
        d_o_f = 1
    else:
        ...
        omega = np.loadtxt(os.path.join('assets', 'ModalAnalysis_Node_EigenVectors_EigenVal.out'),skiprows=1)[2,1]
        d_o_f = 2
    ...
    model.recorder('Node','-file',
                os.path.join('assets', file_names[2]),
    ...
    # Uses the norm of the left hand side solution vector of the matrix equation to
    # determine if convergence has been reached
    model.test('NormDispIncr', tol, max_iter,0,0)
    ...
    model.timeSeries('Path', 2, '-dt', dt, '-values', *accelerogram, '-factor', lamda)

    # allows the user to apply a uniform excitation to a model acting in a certain direction
    model.pattern('UniformExcitation', 3, d_o_f,'-accel', 2)
    ...
    model.rayleigh(a_R, b_R, 0.0, 0.0)

    # Introduces line search to the Newton algorithm to solve the nonlinear residual equation
    model.algorithm('NewtonLineSearch',True,False,False,False,0.8,100,0.1,10.0)
