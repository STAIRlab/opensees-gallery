...
# Define type of analysis:  "pushover" = pushover
	...
	if {$analysisType == "pushover"} {
		set dataDir 2s1b-Pushover-Output;
		file mkdir $dataDir;	# create directory for output data
	}
...
# define extra nodes for plastic hinge rotational springs
	...
	# "a" convention: 1 = left,outer; 2 = left,inner; 3 = right,inner; 4 = right,outer
	# "a" convention: 5 = down,outer; 6 = down,inner; 7 = up,inner; 8 = up,outer 
...
# assign boundary condidtions with "fix" command:  fix nodeID dxFixity dyFixity rzFixity
...
# record response history of all P-Delta column springs (one file for moment, one for rotation)
	recorder Element -file $dataDir/PDcol-Mom-Hist.out -time -region 3 force;
	recorder Element -file $dataDir/PDcol-Rot-Hist.out -time -region 3 deformation;
...
if {$analysisType == "pushover"} { 
	...
	set IDctrlDOF 1;					# degree of freedom (1) of disp read for disp control
	set Dmax [expr 0.1*$HBuilding];		# maximum displacement of pushover: 10% roof drift
	set Dincr [expr 0.01];				# displacement increment

# calculate the lateral loads and create load pattern:  distribute proportional to floor weight and height
	set W2H2 [expr $Floor2Weight*$Floor2];
	set W3H3 [expr $Floor3Weight*$Floor3];
	set sumWiHi [expr $W2H2 + $W3H3];
	set lat2 [expr 1.0/($NBays+1.0) * $WBuilding * $W2H2/$sumWiHi];		# force on each frame node in Floor 2
	set lat3 [expr 1.0/($NBays+1.0) * $WBuilding * $W3H3/$sumWiHi]; 	# force on each frame node in Floor 3
