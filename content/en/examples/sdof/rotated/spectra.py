...
#           system('BandGen')             # how to store and solve the system of equations in the analysis
...
#           test('EnergyIncr',1.0e-6, 100, 0)
            ...
            # Storing Spectra
            omega = 2*np.pi/T
            Rot_Acc = np.dot(Rot_Disp,(omega**2)/gravity)
...
#       GM_SPECTRA.to_csv('Spectra//GM'+str(iEQ)+'_Spectra.txt', sep=' ',header=True,index=False)
        ...
        if Plot_Spectra == True:
            ...
            def plot_spectra(PlotTitle,SpectraType,iGM,fig):
                axes = fig.add_subplot(1, 1, 1)
                axes.plot(GM_SPECTRA['Period(s)'] , GM_SPECTRA[SpectraType] , '.-', label='GM'+str(iGM))
                axes.set_xlabel('Period (sec)',fontsize=30)
                axes.set_ylabel(SpectraType ,fontsize=30)
                axes.set_title(PlotTitle,fontsize=40)
#               axes.tick_params(labelsize= 25)
            ...
            plot_spectra('RotD100 Spectra','RotD100Sa(g)',iEQ, fig)
